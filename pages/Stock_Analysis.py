...
with col1:
    ticker = st.text_input("Stock Ticker", "GOOGL")
with col2:
...
with col1:
    ...
    df[''] = [
        str(market_cap) if market_cap is not None else "N/A",
        str(beta) if beta is not None else "N/A",
        str(eps) if eps is not None else "N/A",
        str(pe_ratio) if pe_ratio is not None else "N/A"
    ]
